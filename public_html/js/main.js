...
(function() {
    ...
    function initQuiz() {
        ...
        var wines = {
            'cabernet': {
                name: 'Legacy Cabernet Sauvignon',
                description: 'Estrutura elegante e taninos aveludados. Notas de frutas negras, especiarias e um toque de carvalho francês.',
                phrase: '"Um vinho que honra a força do patriarca — para quem aprecia intensidade e caráter."'
            },
            'merlot': {
                name: 'Legacy Merlot',
                description: 'Macio e envolvente, com aromas de ameixa madura e chocolate. Taninos sedosos e final persistente.',
                phrase: '"A suavidade paterna em cada gole — perfeito para momentos de contemplação."'
            },
            'marselan': {
                name: 'Legacy Marselan',
                description: 'Intenso e complexo, com personalidade única. Frutas vermelhas maduras, pimenta e notas defumadas.',
                phrase: '"A expressão da ousadia — para quem busca algo fora do comum."'
            },
            'cuvee-blanc': {
                name: 'Cuvée Blanc',
                description: 'Perlage fino e persistente. Aromas de maçã verde, flores brancas e brioche. Frescor elegante.',
                phrase: '"A delicadeza de Erene em cada bolha — celebre os momentos que importam."'
            },
            'cuvee-rose': {
                name: 'Cuvée Rosé',
                description: 'Rosa salmão delicado, bolhas finas e elegantes. Notas de morango, framboesa e pétalas de rosa.',
                phrase: '"A ternura em cada gole — leve, refrescante e inesquecível."'
            },
            'legado': {
                name: 'Cuvée Legado',
                description: 'O ápice da nossa produção. Blend harmonioso de Cabernet Sauvignon e Marselan. Complexidade extraordinária.',
                phrase: '"O legado de três gerações em uma única garrafa — para quem reconhece a verdadeira excelência."'
            }
        };
        ...
        var progressLines = quizContainer.querySelectorAll('.quiz-progress-line');
        ...
        function showQuestion(num) {
            ...
            var target = quizContainer.querySelector('[data-question="' + num + '"]');
            if (target) {
        ...
        function showResult() {
            ...
            var winnerKey = Object.keys(scores).reduce(function(a, b) {
                return scores[a] >= scores[b] ? a : b;
            });

            var wine = wines[winnerKey];

            document.getElementById('quiz-result-name').textContent = wine.name;
            document.getElementById('quiz-result-description').textContent = wine.description;
            document.getElementById('quiz-result-phrase').textContent = wine.phrase;
        ...
        function handleOptionClick(e) {
            ...
            var scoresStr = option.getAttribute('data-scores');
            if (scoresStr) {
                scoresStr.split(',').forEach(function(pair) {
                    var parts = pair.split(':');
                    var wineKey = parts[0].trim();
                    var points = parseInt(parts[1], 10);
                    if (scores[wineKey] !== undefined) {
                        scores[wineKey] += points;
                    }
                });
            }
        ...
        quizContainer.querySelectorAll('.quiz-options').forEach(function(optGroup) {
            optGroup.addEventListener('click', handleOptionClick);
        });
